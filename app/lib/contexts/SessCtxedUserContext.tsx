...
type SessCtxedUserContextValue = {
  user: User | null;
...
const SessCtxedUserContext = createContext<SessCtxedUserContextValue>({
  user: null,
...
export const SessCtxedUserProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  return (
    <SessCtxedUserContext.Provider value={{ user, isUserFirstLoading, updateSessionUser, refreshSessionUser }}>
      {children}
    </SessCtxedUserContext.Provider>
  );
};

export const useSessCtxedUserContext = () => {
  const context = useContext(SessCtxedUserContext);
  if (!context) {
