...
type BookCriticalDropdownProps = {
    bookId: string
    className?: string
}

export const BookCriticalDropdown = ({
    bookId,
    className="h-[2rem]"
}: BookCriticalDropdownProps) => {
    ...
    const handleDelete = async () => {
        ...
        try {
            const res = await deleteBookAndInPagesASF(bookId);
            if (!res) {
                throw new Error("Something went wrong while deleting this book.");
            }

            addToast(makeToastOrder(SUCCESS.DELETE_BOOK_SUCCESS, true));
        } catch (error) {
            ...
            console.error(error);
            addToast(makeToastOrder(ERROR.DELETE_BOOK_SOMETHING_ERROR, false));
        } finally {
            setIsDeleting(false);
        }
    }
            ...
            <div className='fixed inset-0 p-[1rem] z-[50]'>
                ...
                >
                    ...
                    <p className="whitespace-pre-wrap text-center p-[0.5rem]">
                        Are you sure you want{'\n'}to delete this book and the pages in the book?
                    </p>
